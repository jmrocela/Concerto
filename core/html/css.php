...
// do a 'none' to 'transparent' background fix
foreach ($colors['background'] as $key => $value) {
	if ($value == 'none') {
		$colors['background'][$key] = 'transparent';
	}
}
...
// Width Assignments
$width = array(
		'container' => (get_option('concerto_' . $stage . '_design_layout_columns_width_content') + get_option('concerto_' . $stage . '_design_layout_columns_width_sidebar1') + get_option('concerto_' . $stage . '_design_layout_columns_width_sidebar2')) + (get_option('concerto_' . $stage . '_design_page_padding') * 4) + 20,
		'content' => get_option('concerto_' . $stage . '_design_layout_columns_width_content'),
		'sidebar1' => get_option('concerto_' . $stage . '_design_layout_columns_width_sidebar1'),
		'sidebr2' => get_option('concerto_' . $stage . '_design_layout_columns_width_sidebar2')
	);
...
#header{border-bottom:<?php echo $borders['header-bottom']; ?>px solid <?php echo $colors['borders']['common']; ?>;background:<?php echo $colors['background']['header']; ?><?php echo ($colors['background']['header-image']) ? " url('" . $colors['background']['header-image'] . "') no-repeat center scroll": ''; ?>; }
...
.sidebars{border-left:<?php echo $borders['container']; ?>px solid <?php echo $colors['borders']['common']; ?>;width:<?php echo get_option('concerto_' . $stage . '_design_layout_columns_width_sidebar1') - get_option('concerto_' . $stage . '_design_layout_columns_width_sidebar2'); ?>px;font-family:<?php echo $fonts['sidebar']; ?>;font-size:<?php echo $sizes['sidebar']; ?>px;line-height:<?php echo $sizes['sidebar'] + 7; ?>px;}
